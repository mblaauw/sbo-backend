...
from schemas import SkillCategory, Skill
...
def get_candidate_dashboard(
    ...
    try:
        ...
        assessment_results = db.query(
            ...
            AssessmentResult.user_id == candidate_id
        ).order_by(
...
def get_role_dashboard(
    ...
    try:
        ...
        requirements_query = db.query(
            ...
            Skill, RoleSkillRequirement.skill_id == Skill.id
        ).filter(
...
def get_organization_dashboard(
    ...
    try:
        ...
        categories = db.query(
            ...
            Skill, SkillCategory.id == Skill.category_id
        ).group_by(
            SkillCategory.name
        ).all()
        ...
        role_skills = db.query(
            ...
            RoleSkillRequirement, Skill.id == RoleSkillRequirement.skill_id
        ).group_by(
            Skill.id, Skill.name
        ).order_by(
            db.desc('role_count')
        ...
        recent_assessments = db.query(
            ...
            UserModel, AssessmentResult.user_id == UserModel.id
        ).join(
            Assessment, AssessmentResult.assessment_id == Assessment.id
        ).order_by(
            AssessmentResult.completed_at.desc()
